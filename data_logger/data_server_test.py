...
class DataServerTestCase(unittest.TestCase):
	...
	def testUpload_setsErrorWhenResponseDataIsInvalidJSON(self):
		self._mock_http_conn_via_create_http_conn()
		create_http_conn_value.getresponse_value.status = 200
		create_http_conn_value.getresponse_value.read_value = "{...invalid JSON}"
		self.srv.upload(self.expectedRequestBody)
		self.assertEqual(True, self.srv.error.startswith("JSON Error"))
	...
	def testUpload_setsErrorWhenResponseDataIsErrorIsTrue(self):
		errmsg = "Some database error"
		expected = "Server Error: %s" % errmsg
		self._mock_http_conn_via_create_http_conn()
		create_http_conn_value.getresponse_value.status = 200
		create_http_conn_value.getresponse_value.read_value = "{\"Message\":\"%s\", \"IsError\":true}" % errmsg
		self.srv.upload(self.expectedRequestBody)
	...
	def testUpload_returnsTrue(self):
		self._mock_http_conn_via_create_http_conn()
		create_http_conn_value.getresponse_value.status = 200
		create_http_conn_value.getresponse_value.read_value = "{\"Message\":null, \"IsError\":false}"
